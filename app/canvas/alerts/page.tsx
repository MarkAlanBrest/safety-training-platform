...
export default async function CourseAlertsPage({ searchParams }: Props) {
  ...
  const config = courseId ? await getCourseAlertConfig(courseId) : null;
  ...
  return (
    <main className="course-alerts-page">
      {courseId ? (
        <CourseAlertsViewer
          courseId={courseId}
          initialCourseName={courseName || config?.courseName || null}
          initialStudentName={studentName}
          initialBannerMessage={config?.bannerMessage}
