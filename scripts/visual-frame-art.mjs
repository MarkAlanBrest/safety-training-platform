...
function fillBackground(ctx, width, height, top, bottom) {
...
function drawPerson(ctx, x, y, scale, shirt, pants) {
  ctx.save();
  ctx.translate(x, y);
...
function drawTable(ctx, x, y, width) {
...
function drawHarassmentFrame(ctx, width, height, frameIndex) {
  ...
  if (frameIndex === 0) {
    ...
    ctx.beginPath();
    ctx.rect(width * 0.48, height * 0.28, width * 0.22, height * 0.14);
    ctx.fill();
...
function drawLadderFrame(ctx, width, height, frameIndex) {
...
export function buildFramePicture(theme, frameIndex, frameCount, title = "") {
  ...
  return {
    ...
    moments: lessonPlan.moments.map((moment) => {
      ...
      return {
        ...
        sourceImage: nextFrames[0]?.sourceImage || null,
        sourceImageAlt: moment.sourceImageAlt || moment.title,
